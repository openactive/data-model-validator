...
class ApplyRules {
    ...
    for (const fieldValue of fieldsToTest) {
      if (typeof fieldValue === 'object' && fieldValue !== null && !(fieldValue instanceof Array)) {
        ...
        if (
          ...
          const subModelErrors = this.applyModelRulesSync(
            rules,
            newNodeToTest,
          );
  ...
  static applyModelRulesSync(rules, nodeToTest) {
    ...
    for (const rule of rules) {
      ...
      errors = errors.concat(rule.validateSync(nodeToTest));
    }
    for (const field in nodeToTest.value) {
      if (Object.prototype.hasOwnProperty.call(nodeToTest.value, field)) {
        ...
        errors = errors.concat(this.applySubModelRulesSync(rules, nodeToTest, field));
      }
    }
    return errors;
  }
...
function validateSync(value, options) {
  ...
  for (const rule of rawRuleObjects) {
    const response = rule.validateSync(valueCopy);
    errors = errors.concat(response.errors);
    if (typeof response.data !== 'undefined') {
      valueCopy = response.data;
    }
  ...
  for (const valueToTest of valuesToTest) {
    ...
    errors = errors.concat(
      ApplyRules.applyModelRulesSync(
        coreRuleObjects,
        nodeToTest,
      ),
    );
