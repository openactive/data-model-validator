...
function applyModelRules(rules, data, model, parent, path) {
    ...
    for (let rule of rules) {
        if (rule.isModelTargeted(model)) {
            errors = processErrors(rule.validateModel(data, model, parent), errors, path);
        }
    }
    for (let field in data) {
        errors = errors.concat(applyFieldRules(rules, data, field, model, parent, path));
...
function applyFieldRules(rules, data, field, model, parent, path) {
    let errors = [];
    for (let rule of rules) {
        if (rule.isFieldTargeted(field, model)) {
            errors = processErrors(rule.validateField(data, field, model, parent), errors, path);
        }
    }
    return errors;
}
